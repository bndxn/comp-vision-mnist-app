...
# Pytorch has two primitives to work with data: the `DataLoader` and `Dataset`. The latter stores the samples and the former wraps an iterable around the `Dataset`. 
...
def train(dataloader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        # why less than 100? The batch size is 64
        # this prints a result every 100 items 
            loss, current = loss.item(), (batch + 1) * len(X)
            print(f"batch number: {batch}, loss: {loss:>7f} [{current:>5d}/{size:>5d}]")
